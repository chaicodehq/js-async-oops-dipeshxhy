...
export async function raceBooking(trainNumbers, passenger, date, classType) {
  try {
    const results = trainNumbers.map(
      async (train) => await bookTicket(passenger, train, date, classType),
    );
    const res = await Promise.any(results);
