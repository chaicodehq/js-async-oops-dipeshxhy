...
export class Batsman extends Player {
  ...
  getProfile() {
    return {
      ...
      role: "batsman",
      totalRuns: this.innings.reduce((total, inning) => total + inning.runs, 0),
      inningsPlayed: this.innings.length,
    };
  }
}
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalSpell = this.spells.reduce(
      (total, spell) => total + spell.economy,
      0,
    );
    return totalSpell / this.spells.length;
  }
